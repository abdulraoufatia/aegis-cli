...
class Database:
    ...
    def archive_audit_events_by_count(
        self,
        archive_path: Path,
        max_rows: int,
    ) -> int:
        """Archive oldest events to keep at most *max_rows* in the main DB.
        ...
        self._db.execute(f"DELETE FROM audit_events WHERE id IN ({placeholders})", row_ids)  # noqa: S608
        self._db.commit()
