...
The following is the canonical definition of the adapter contract as of v1.0.0. All adapters in `src/atlasbridge/adapters/` must subclass `BaseAdapter` and implement every abstract method. The `PromptEvent` and `Reply` models live in `src/atlasbridge/core/prompt/models.py`.
...
| `tool_name` | `str` | Short identifier (e.g. `"claude"`). Used in config and CLI output. |
| `description` | `str` | Human-readable label shown in `atlasbridge adapter list`. |
| `min_tool_version` | `str` | Minimum supported version of the wrapped tool. |

### Abstract Methods (must implement)

| Method | Signature | Description |
|--------|-----------|-------------|
| `start_session` | `async (self, session_id: str, command: list[str], env: dict \| None = None, cwd: str = "") -> None` | Launch the CLI tool and open the PTY session. |
| `terminate_session` | `async (self, session_id: str, timeout_s: float = 5.0) -> None` | Terminate the CLI tool process gracefully, then forcibly. |
| `read_stream` | `async (self, session_id: str) -> bytes` | Return the next chunk of raw PTY output bytes. Returns `b""` on EOF. |
| `inject_reply` | `async (self, session_id: str, value: str, prompt_type: str) -> None` | Inject a reply value into the CLI's stdin. |
| `await_input_state` | `async (self, session_id: str) -> bool` | Return True if the TTY is blocked on read (Signal 2 heuristic). |

### Optional Methods (have defaults)

| Method | Signature | Default |
|--------|-----------|---------|
| `snapshot_context` | `(self, session_id: str) -> dict[str, Any]` | Returns `{}`. |
| `get_detector` | `(self, session_id: str) -> Any` | Returns `None`. |
| `healthcheck` | `(self) -> dict[str, Any]` | Returns `{"status": "ok", "adapter": self.tool_name}`. |

### Adapter Registry

| Method | Description |
|--------|-------------|
| `AdapterRegistry.register(name)` | Decorator to register an adapter class. |
| `AdapterRegistry.get(name)` | Retrieve an adapter class by name. |
| `AdapterRegistry.list_all()` | Return `dict[str, type[BaseAdapter]]` of all registered adapters. |

### Frozen Registered Adapters

| Name | Class | Status |
|------|-------|--------|
| `claude` | `ClaudeCodeAdapter` | Production |
| `claude-code` | `ClaudeCodeAdapter` (alias) | Production |
| `openai` | `OpenAICLIAdapter` | Registered |
| `gemini` | `GeminiCLIAdapter` | Registered |
| `custom` | `CustomAdapter` | Registered |
