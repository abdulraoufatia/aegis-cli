...
The `atlasbridge setup` command provides the first-run experience. It must be:
...
│     If ~/.atlasbridge/config.toml exists:                           │
│       "AtlasBridge is already configured. Use --force to re-run."   │
│       → exit 0 (unless --force)                               │
└───────────────────────────────────────────────────────────────┘
     │
     ▼
...
│     b. Check ATLASBRIDGE_TELEGRAM_BOT_TOKEN env var                 │
...
│     b. Check ATLASBRIDGE_TELEGRAM_ALLOWED_USERS env var             │
│     c. Prompt user (interactive mode)                         │
...
│     b. "✅ AtlasBridge is connecting. This is a test message."       │
...
│     b. Write selected policy to ~/.atlasbridge/policy.toml          │
...
│     a. Create ~/.atlasbridge/ directory (0700)                      │
│     b. Write ~/.atlasbridge/config.toml (0600)                      │
│     c. Initialize ~/.atlasbridge/atlasbridge.db (WAL mode)                │
│     d. Run schema migrations                                  │
│     e. Write ~/.atlasbridge/policy.toml (0600)                      │
└───────────────────────────────────────────────────────────────┘
     │
     ▼
┌───────────────────────────────────────────────────────────────┐
...
```
~/.atlasbridge/                       (0700 — dir owner-only)
├── config.toml                 (0600 — owner read/write only)
├── policy.toml                 (0600)
├── atlasbridge.db                    (0600)
├── atlasbridge.log                   (0600, created on first start)
├── atlasbridge.pid                   (0600, created on start)
...
# Generated by: atlasbridge setup
...
[database]
path = "~/.atlasbridge/atlasbridge.db"
```
...
```toml
# ~/.atlasbridge/policy.toml — Strict preset
[policy]
...
[[policy.rules]]
name = "block-atlasbridge-config"
match = { path_pattern = ".*\atlasbridge/.*" }
action = "deny"
priority = 1
...
```toml
# ~/.atlasbridge/policy.toml — Balanced preset
[policy]
default_action = "require_approval"
...
atlasbridge setup \
  --non-interactive \
  --telegram-token "$ATLASBRIDGE_TELEGRAM_BOT_TOKEN" \
  --allowed-users "$ATLASBRIDGE_TELEGRAM_ALLOWED_USERS" \
  --policy strict \
  --json
```
...
```
⚠️  AtlasBridge is already configured. Re-running setup will:
    - Overwrite ~/.atlasbridge/config.toml
    - Overwrite ~/.atlasbridge/policy.toml
...
| Missing dependency | 7 | `pip install atlasbridge` |
...
| Cannot create ~/.atlasbridge/ | 5 | Check disk space and home dir permissions |
| Setup aborted by user | 1 | Run `atlasbridge setup` again |

---

## Idempotency

`atlasbridge setup` is idempotent when run with `--force`:
- Re-running creates a new config but does NOT wipe the database
- All previous approval history is preserved
- New bot token takes effect on next `atlasbridge start`
